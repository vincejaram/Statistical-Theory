...
skew_scores
ses_scores <- sqrt((6*n*(n-1))/((n-2)*(n+1)*(n+3)))
ses_scores
kurt_scores <- kurtosis(scores)
kurt_scores
sek_scores <- 2*ses_scores*(sqrt((n^2-1)/((n-3)*(n+5))))
sek_scores
...
df <- data.frame(values=c('Number of Scores','Mode','Median','Mean','Std. Deviation','Variance','Skewness','Std. Error of Skewness','Kurtosis','Std. Error of Kurtosis','Minimum','Maximum','25th percentile','50th percentile','75th percentile','90th percentile','95th percentile'), results=c(n,mode_scores,median_scores,mean_scores,sd_scores,var_scores,skew_scores,ses_scores,kurt_scores,sek_scores,min_scores,max_scores,q1,q2,q3,d9,p95))
df